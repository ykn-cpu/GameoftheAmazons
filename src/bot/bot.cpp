...
//// 区域归属评估：返回我方可达格子数减去对方可达格子数
int territory_score(int mycolor) {
    int vis[GRIDSIZE][GRIDSIZE] = { 0 };
    queue<pair<int, int>> q_my, q_op;
    // 标记起点
    for (int x = 0; x < GRIDSIZE; ++x) {
        for (int y = 0; y < GRIDSIZE; ++y) {
    ...
    // BFS扩展对方
    while (!q_op.empty()) {
        pair<int, int> cur = q_op.front(); q_op.pop();
        int x = cur.first, y = cur.second;
        for (int d = 0; d < 8; ++d) {
            for (int step = 1; step < GRIDSIZE; ++step) {
                int nx = x + dx[d] * step, ny = y + dy[d] * step;
...
int evaluate() {
    ...
    score += 35 * territory_score(mycolor);
    return score;
}
const int MAX_SEARCH_TIME = 950; // ms
bool timeOut = false;
...
void aiMove() {
    ...
    sort(moves.begin(), moves.end(), [&](const Move& a, const Move& b) {
        int backup[GRIDSIZE][GRIDSIZE];
        memcpy(backup, gridInfo, sizeof(gridInfo));
        ProcStep(a.x0, a.y0, a.x1, a.y1, a.x2, a.y2, mycolor, false);
        int scoreA = fastmobility(mycolor) - fastmobility(-mycolor) +
            3 * blockscore(mycolor) + 35 * territory_score(mycolor);
        memcpy(gridInfo, backup, sizeof(gridInfo));
        ProcStep(b.x0, b.y0, b.x1, b.y1, b.x2, b.y2, mycolor, false);
        int scoreB = fastmobility(mycolor) - fastmobility(-mycolor) +
            3 * blockscore(mycolor) + 35 * territory_score(mycolor);
        memcpy(gridInfo, backup, sizeof(gridInfo));
    ...
    for (int depth = 1; depth <= 5; depth++) {
        ...
        for (size_t i = 0; i < moves.size(); i++) {
            memcpy(backup, gridInfo, sizeof(gridInfo));
            ProcStep(moves[i].x0, moves[i].y0, moves[i].x1, moves[i].y1, moves[i].x2, moves[i].y2, mycolor, false);
            int score = minimax(-mycolor, -1e9, 1e9, 1, depth);
            memcpy(gridInfo, backup, sizeof(gridInfo));
            if (score > bestscore) {
                bestscore = score;
                bestmoves.clear();
    ...
    cout << lastBestMove.y0 << " " << lastBestMove.x0 << " "
        << lastBestMove.y1 << " " << lastBestMove.x1 << " "
        << lastBestMove.y2 << " " << lastBestMove.x2 << endl;
}
int main() {
    startTime = steady_clock::now(); // 初始化搜索起始时间
